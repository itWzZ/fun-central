<template>
      ...
      <transition-group name="fade-slide">
        <div v-for="(item,index) in arrPage "
          ...
          <div class="w-full h-full flex items-center" style="font-size: 15px;">
            ...
            <edit-outlined @click.stop="handleOpenModal(item.id,item.title)" class="ml-3 text-black  editor-icon  hover:text-gray-500"/>
          </div>
          <a-popconfirm
              title="确定删除此页面?"
              ok-text="确定"
          ...
          >
            <div style="transform: translateX(-60px)" class="flex item-center  cursor-pointer hover:text-gray-600 text-black"
  ...
  <a-modal v-model:visible="visible" @ok="handleOk" :afterClose="afterClose">
    <template #title><span class="font-semibold">页面重命名</span></template>
    <span class="mr-1">页面标题：</span>
    <a-input class="w-4/5" v-model:value="pageTitle.pageName" :maxlength="30" placeholder="请输入页面名称"/>
  </a-modal>
</template>
...
const handleChangePage = (id: any, idx: number, name: string) => {
...
const afterClose = () => {
  if (pageTitle.pageName === '') {
    store.actSetPage({id: pageTitle.id, title: '请输入页面名称'})
  }
}
</script>
